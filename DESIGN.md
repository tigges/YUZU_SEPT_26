...
The homepage is a version gallery. **Wireframe** (`?v=wireframe`) is Version 1: a draggable page-order sketch. → sends a section into the sub-page column of the block above; ← brings it back. Off-site links (Phorest, Maps, socials) and existing sub-pages (price list, patch-test PDF, T&Cs) stay as chips. A news ticker sits under the header; take-home retail is a quiet desk shelf (Nashi / K2.0 — no cart). Blocks dragged below the footer are archived (grey). Meet the stylists, gift vouchers, and first-visit FAQ start in the archive. Earthy is `?v=earthy`. **Links** (`?v=links`) is a desk of live URLs and PDFs (price list + patch testing), plus the old Ueni site marked take-offline. T&Cs and offers are Wix pages — there is no offers PDF.
