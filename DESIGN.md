...
The homepage is a version gallery. **Earthy** (`?v=earthy`) is Version 1, then **Sanctuary**, **Clean**, **Round**, and **Studio**. **Wireframe** (`?v=wireframe`) is a draggable page-order sketch. A hero carousel (hero + Book, offers, patch test, new-customer cut), a customer photo-upload banner, and a WhatsApp widget sit on the page; the price list nests under services. Welcome, follow, enquiry form, and the social bar start in the archive. → sends a section into the sub-page column of the block above; ← brings it back. Header and footer stay as chrome. The news ticker can be archived. External links (Phorest, Maps, socials, PDFs) live in a tray: drag onto a section or nested sub-page, drag again to place the same link twice, × to remove. A news ticker sits under the header; take-home retail is a quiet desk shelf (Nashi / K2.0 — no cart). Blocks dragged below the footer are archived (grey). Meet the stylists, gift vouchers, and first-visit FAQ also start in the archive. **Wix** (`?v=wix`) clones the live homepage. **Hairlust** (`?v=hairlust`) clones the hairlust.com homepage layout with Yuzu/Dickens Yard content. **Links** (`?v=links`) is a desk of live URLs and PDFs (price list + patch testing). T&Cs and offers are Wix pages — there is no offers PDF. **HL clone** (`?v=hlclone`) is a clean replica of hairlust.com itself (Hairlust branding, products, and copy) at the end of the gallery.
